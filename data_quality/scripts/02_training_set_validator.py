...
MAX_SPEAKER_DOMINANCE = 0.50  # no single speaker > 50% of training data
MIN_TRAIN_EVAL_RATIO  = 2.0   # train must be at least 2x eval size
MIN_VOCAB_SIZE        = 20    # degenerate dataset if vocab too small
...
def check_transcript_coverage(splits):
    ...
    for split_name, manifest in splits.items():
        ...
        for r in manifest.get("records", []):
            text = r.get("transcript_normalized", r.get("transcript", ""))
